...
rem + the queue worker (mail + SLA engine; requires REDIS_URL in backend\.env).
...
start "HR Worker" cmd /k "cd /d %~dp0backend && npm run worker"
start "HR Frontend :3000" cmd /k "cd /d %~dp0frontend && npm run dev"

echo Three windows opened: backend (:4000), worker (queues), frontend (:3000).
